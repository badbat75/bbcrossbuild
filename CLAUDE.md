...
# Regenerate the branch-tracking patches for gcc/binutils/glibc/gdb under packages/lfs/<pkg>/patches/ and rewrite the version case block in packages/lfs/<pkg>/package.env
utilities/update_patches gcc 14.2.0
# Check the package directories (layout, bash -n, shellcheck, removed variables, BUILD_PROCESS, PATCHES and PKG_DEPS resolution)
...
- A package is a directory: `package.env` (variable assignments, Bash evaluated with `PKG_TARGET`, `TOOLCHAIN`, `HARCH`, `HM`, `PLATFORM_NAME`, `INSTALL_*`, `GCC_VER` etc. already set, so recipes routinely `case` on them, see `packages/lfs/wpa2` for per-target variants, `packages/lfs/gcc` for per-version patch selection), plus optional `prebuild.sh`, `postbuild.sh`, `build.sh` (`BUILD_PROCESS=custom`), `postinstall.sh`, `files/` and `patches/`. `packages/template/` documents all of it; `utilities/pkg_lint` checks it.
...
- `utilities/`: host helpers (`deptool`, `crossgdb`, `crossldd`, `qemu_cmdgen`, `fs_manager`, `aws_create_infrastructure`, bootstrap scripts, container scripts).
