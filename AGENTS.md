...
What the project decides about the system it builds, as directives instead of files written by hand: `set_hostname`, `set_locale`, `configure_network` (the `.network` of a link, DHCP or static), `set_network_wait_online`, `configure_wireless` (the key of the network derived from a passphrase read on stdin, never a command line), `configure_ssh` and `generate_ssh_keys`, `add_unit_dropin`, `enable_service` / `disable_service` / `mask_service` / `preset_service`, `set_service_preset`, `set_default_target`, `add_user`, `set_password`, and `add_postinstall_command` for anything else.
