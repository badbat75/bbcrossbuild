...
# Latest upstream versions (GitHub tags or the archive directory of PKG_URL, Anitya as second opinion; a GitHub
...
`platforms/*.conf` define the target triple (`HOS`, `HM`, `HLIBC` combine into `HARCH`, e.g. `aarch64-linux-gnu`), compiler tuning (`HMARCH`, `HMCPU`, `HMFPU`, `HMFLOATABI` become `-march`, `-mtune`, `-mfpu`, `-mfloat-abi` of `PLATFORM_CFLAGS`; `HMGCCPARAMS` adds compiler options, `HMLDPARAMS` linker options of target builds that rustc also gets as `-C link-arg`, `HMARCH_RUST` the `-C target-feature` next to `-C target-cpu=${HMCPU}`), kernel settings (`KERNEL_ARCH`, `KERNEL_DEFCONFIG`, `KERNEL_IMAGE`, `KERNEL_NAME`, `KERNEL_DTBS`) and QEMU settings used by `utilities/qemu_cmdgen`. `HARCH_LIB=64` selects `lib64`; leaving it empty with `MULTIARCH=1` (the default) selects Debian-style `lib/<triple>` multiarch directories. `-march` grants the instruction set and `-mtune` only chooses the scheduling, so `HMARCH` names only extensions the CPU really has (`/proc/cpuinfo` of the board): `+sve` on the Pi 3 put SVE instructions into ld.so and every userspace binary faulted at `/init`, and the Pi 3 and Pi 4 have `+crc` but no crypto. The cross gcc is configured with `--with-arch=${HMARCH}` too, and rustc's `target-cpu=cortex-a53`/`cortex-a72` assumes crypto (hence `-aes,-sha2` in `HMARCH_RUST`). A change of a platform file invalidates no status file: the project directory has to be emptied.
...
- The package name and its default target are `PKG_FULLNAME = <name><PKG_SUFFIX>_<PKG_VER>`. Directories like `rpi-kernel_7.2` or `camilladsp_1` carry the major version in the name and complete it with `PKG_SUFFIX`.
- `PKG_DEPS` are built recursively first. A dependency written as `lfs/bison:native` builds into the global toolchain; `:cross` builds into the per-platform toolchain; no suffix builds for the target. Native and cross parents propagate their target to their own dependencies. The dependencies are walked for a package already built too (quietly: the console only gets `Package <name>, required by <package>` for one rebuilt), so a dependency whose recipe changed is rebuilt; its dependents are not (their status is their own recipe checksum: `--force`). A package whose `PKG_CHECK` probe succeeds does not walk its dependencies. Every package is checked once per run: `build` records what it built or found built in `BUILD_CHECKED_FILE` (`/tmp/bbxb_checked.<pid>`, created by `bbxb`; unset, every call checks), and `--force` bypasses the list. A probe that never succeeds therefore rebuilds its package on every run that reaches it (lfs/groff `-V`, lfs/cargo-c `+cargo` suffix, both fixed in September 2026).
- Three targets, three prefixes: `native` installs into `GLOBAL_TOOLCHAIN_PATH` (requires `PKG_CHECK`, never records status), `cross` installs into `TOOLCHAIN_PATH`, `target` (default) installs into a staging dir `PKG_PKGPATH`, has `.la` files stripped, is packed by `create_sfx_package` into `<platform>/packages/<fullname>.sfx` (a self-extracting installer with `--with-dev`, `--with-conf`, `--dest`, `--no-postinst` options, which runs the `postinst_scripts/` of the package when it installs on `/` and leaves them in `<dest>/postinst_scripts` otherwise) and rsync'd into `BIN_PATH`.
- Status is the md5 of the whole recipe directory (`recipe_checksum`: names, symlink targets and contents, `*.bak`/`*.orig`/`*.rej` excluded). Editing the recipe, a script, a static file or a patch automatically invalidates it; `PKG_CHECK="<command>"` replaces the status check with a probe. `--temporary`, `--no_save_status`, `--force`, `--keep_builddir`, `--clean_builddir`, `--toolchain gnu|llvm` are the useful flags. `PKG_KEEPBUILDDIR=1` keeps the build directory and the prepared sources between builds (`sources_key` marker `.bbxb-sources` in the sources, `builddir_key` stamp `.bbxb-environment` in the build directory, which is emptied when the stamp changes; `stale_builddirs` removes the trees of other versions): `lfs/llvm` shares one tree between its target flavors.
...
`setbuildenv`, `settcenv` and `create_environment_source` (all in `build.functions`) compute the compiler, flag and pkg-config environment for the three targets; `setllvmpollyenv` adds Polly flags. Look there first when a package picks up the wrong sysroot, linker or `PKG_CONFIG_LIBDIR`. A target build and the gcc target libraries of `toolchain.functions` compile with the source path maps of `host_path_maps` (`-ffile-prefix-map` in `CFLAGS`/`CXXFLAGS`/`CPPFLAGS` and `CFLAGS_FOR_TARGET`, `--remap-path-scope=object --remap-path-prefix` in `RUSTFLAGS`): the data directory is recorded as `/usr/src/bbxb` and the sysroot as the image, so `__FILE__`, debug information and Rust panic locations carry no host path; the build logs keep the real paths. The command lines of a target build name no host path either, because build systems copy them into binaries (`openssl version -a`, vim `:version`, `sudo -V`): the programs go by name through `PATH` (the compilers, `AR`..., `sccache`), and the compiler carries the sysroot, the maps, the multiarch `-rpath-link` and the gcc library `-L` in a configuration of its own: a cross gcc whose sysroot is `BIN_PATH` has a specs file (`setup_gcc_specs` in `toolchain.functions`, content from `gcc_host_path_specs`), the platform toolchain has its own copy of clang, `${TOOLCHAIN_PATH}/llvm-<ver>/bin/<HARCH>-clang[++]`, with `<normalized triple>.cfg` next to it (`setup_clang_config`, content from `clang_host_path_config`; clang reads a configuration file only next to its real executable, so a link does not work). Then `setbuildenv` sets `CC_HOST_PATH_CONFIG=1` and `settcenv` drops `--target`, `--sysroot`, the maps and those link flags from `CFLAGS`/`LDFLAGS`; `configmake` passes a bare `--with-sysroot` to gcc (libtool asks it) and none to clang; `cmakebuild` keeps `CMAKE_SYSROOT` but empties the `--sysroot` option CMake adds (`CMAKE_USER_MAKE_RULES_OVERRIDE`). After packaging, `build` logs `WARNING: host paths in <n> files of <package>` for the files of the staging directory that still name the data directory or the checkout (`find_host_paths`). The LTO bytecode always names the host, in compressed streams no search finds (gcc: the unmapped working directory or absolute source name, GCC PR 108534; clang: the absolute `source_filename`), so before that `build` runs `strip_lto_objects`: without static libraries (`BUILD_LIBSTATIC`/`PKG_OVERRIDESTATIC` 0) a `lib<name>.a` with bytecode next to its `.so` goes; with them `settcenv` compiles with `-ffat-lto-objects` and the bytecode sections are removed; slim objects left stop the build (a static-only library such as the lfs/Tcl stubs or the lfs/dhcp libraries needs `PKG_OVERRIDESTATIC=1`). A python module installs with `--no-compile` and is byte-compiled afterwards (`pythonbuild`), because pip writes the path where it compiles into the `pyc`. Target builds run pkgconf with the FDO sysroot rules (`PKG_CONFIG_FDO_SYSROOT_RULES=1`): the sysroot is prepended to `-I`/`-L` only, and `--variable` returns the path of the image, which is what install directories and run time paths need. The cross pkgconf (`lfs/pkgconf:cross`) has the include and library directories of the sysroot as its system directories, so `--cflags` and `--libs` leave them out: the compiler finds them through `--sysroot`, and no build copies a `-L`/`-I` of the sysroot into its own files (`.pc`, `*-config`) or turns it into a RUNPATH. A pc variable naming a program or a file that later builds read carries `${pc_sysrootdir}`, written by the `postbuild.sh` of the recipe that installs it (lfs/GLib, lfs/gobject-introspection, lfs/wayland, lfs/gdk-pixbuf, lfs/xorg-sgml-doc-tools); a build that reads a standard variable as an input gets the sysroot through its own option or the `PKG_CONFIG_<PACKAGE>_<VARIABLE>` override of pkgconf (lfs/libX11, lfs/gobject-introspection, lfs/Mesa).

### Toolchain bootstrap (toolchain.functions)

`setup_full_toolchain` runs, in order: `setup_rust`, then `lfs/sccache:native` (started as a server and installed as `CCWRAPPER` for everything that follows), make, autotools, pkgconf (native and cross), bison, cmake, `lfs/mold:cross` when `GCC_DEFAULT_LD` or `LLVM_DEFAULT_LD` is `mold` (until then `linker_for_build` in `build.functions` links the native and cross builds with bfd or lld; the cross build installs `<HARCH>-ld.mold` for the cross gcc), `setup_binutils`, `setup_gcc` (host compiler plus target libraries), the toolchain wrapper, ninja, `setup_llvm` when `--with-llvm` is passed or `TOOLCHAIN=llvm`, a native Python and meson, and finally the cross Python only with `--with-python`. Most steps are ordinary `build ...:native` / `:cross` calls on `packages/lfs/*.pkg` recipes. Versions come from `setenv` (`BINUTILS_VER`, `GCC_VER`, `LLVM_VER`, `PYTHON_VER`, `KERNEL_VER`, `GLIBC_VER`, autotools versions). Patches for gcc, binutils, glibc and gdb are release-branch diffs named `packages/lfs/<pkg>/variants/version/<ver>/patches/00-<pkg>_<ver>+<shortsha>.patch`, generated by `utilities/update_patches`; adding a new toolchain version means running that script (an unknown version builds unpatched). The glibc variants also carry `10-glibc-<ver>-fhs-1.patch`, the LFS FHS patch. `PKG_VER` of these recipes is the plain version: the patch content is part of the recipe checksum. `build lfs/gcc:binaries` builds only the host programs (`lfs.prj` does that for the development image).
...
What the project decides about the system it builds, as directives instead of files written by hand: `set_hostname`, `set_locale`, `configure_network` (the `.network` of a link, DHCP or static), `set_network_wait_online`, `configure_wireless` (the key of the network derived from a passphrase read on stdin, never a command line; `--networkmanager` writes a NetworkManager keyfile profile instead), `configure_networkmanager` (the `conf.d` file, `dns=systemd-resolved`), `configure_ssh` and `generate_ssh_keys`, `add_unit_dropin`, `enable_service` / `disable_service` / `mask_service` / `preset_service`, `set_service_preset`, `set_default_target`, `add_user`, `set_password`, and `add_postinstall_command` for anything else.

One rule splits them in two. What is a file is written into the sysroot (`${BIN_PATH}`), where the image, the chroot and the packages built from it find it. What has to run inside the target, because only the target has the databases and the tools it touches (`systemctl`, `useradd`, `chpasswd`), becomes a line of the post install script of the project, `${BIN_PATH}/postinst_scripts/99_osconfig`, which `run_postinstall_scripts` runs as root inside the image after the ones of the packages; the same functions take `--tag <mount tag>` and then act at once on a mounted image, which is what a project needs after `inject_into_mount_tag`, when the post install scripts have already run: the command runs on the build host as root against the root of the image (`systemctl --root`, `useradd --prefix`, `chpasswd --prefix`) with the programs of the global toolchain (`host_on_target`, which builds `lfs/systemd:native` or `lfs/shadow:native` on first use), not in its chroot under emulation. A framework function that needs a program uses the one of a native recipe in the global toolchain (`wpa_passphrase`, `ssh-keygen`, the dracut of `kernelbuild`), never the one of the build host. That script and the preset file of the project (`/etc/systemd/system-preset/00-<project>.preset`) are removed when `osconfig.functions` is sourced, the one moment a run is known to start: the sysroot survives from one build to the next and a directive no longer in the project file must not stay in it. Every function copies what `param2value` parsed into its own variables before calling anything else, because the `_param_` variables are global and the next `param2value` unsets them; `param_list` (`core.functions`) collects the positional ones for the functions that take a list of units.
...
- `utilities/`: host helpers (`deptool`, `crossgdb`, `crossldd`, `qemu_cmdgen`, `aws_create_infrastructure`, bootstrap scripts); `pkg_lint` and `pkg_show` share `utilities/pkgtools.functions`, which sources the framework with the build steps stubbed out and resolves a recipe through `set_target_prefixes` and `apply_recipe_variants`, the same code `build` uses.
