...
2. **libclc for the target LLVM.** LLVM 23 builds libclc only as a runtime (`LLVM_ENABLE_RUNTIMES`),
...
3. **librsvg gdk-pixbuf SVG loader.** The 2.63.0 meson recipe sets `-Dpixbuf-loader=disabled`. The
...
4. **nss.pc library directory.** The `nss.pc` generated by the BLFS standalone patch hardcodes
...
5. **gdk-pixbuf glycin loader.** The 2.44.8 recipe sets `-Dglycin=disabled` (glycin is a Rust
